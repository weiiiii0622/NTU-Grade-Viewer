...
type A1 = ((arg: 1 | 2) => 0) | ((arg: 2 | 3) => 0) extends (arg: infer I) => 0
  ? I
  : "never";
// A1 = 2
...
type UnionToIntersection<U> = (U extends U ? (arg: U) => 0 : "never1") extends (
  arg: infer I
) => 0
  ? I
  : "never2";
type GetLast<U> = UnionToIntersection<
  U extends U ? () => U : "never3"
> extends () => infer R
  ? R
  : "never4";
...
type JoinStringUnion<
  ...
  D extends string,
  S = UnionToTuple<U>
> = S extends string[] ? JoinStringTuple<S, D> : "never";
...
type DictParamsToURL<P extends Record<string, string>, K = keyof P> = [
  K
] extends [never]
  ? ""
  : `?${JoinStringUnion<
